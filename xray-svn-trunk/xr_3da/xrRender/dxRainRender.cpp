...
void dxRainRender::Render(CEffect_Rain &owner, SRainParams* p)
{
	float	factor				= g_pGamePersistent->Environment().CurrentEnv->rain_density;
	if (factor<EPS_L)			return;

  	u32 desired_items			= iFloor	(0.5f*(1.f+factor)*float(p->max_desired_items));
	// visual
	...
	float	b_radius_wrap_sqr	= _sqr((p->source_radius+.5f));
	if (owner.items.size()<desired_items)	{
		...
		while (owner.items.size()<desired_items)	{
			...
			owner.Born				(one,p->source_radius);
	...
	Fvector upper; 	upper.set(Device.vCameraPosition.x,Device.vCameraPosition.y+p->source_offset,Device.vCameraPosition.z);
	...
	for (u32 I=0; I<owner.items.size(); I++){
		...
		if (one.dwTime_Life<Device.dwTimeGlobal)	owner.Born(one,p->source_radius);
		...
		if (wlen>b_radius_wrap_sqr)	{
			...
			if ((one.P.y-vEye.y)<p->sink_offset){
				// need born
				one.invalidate();
			}else{
				...
				if (src_plane.intersectRayPoint(one.P,inv_dir,src_p)){
					...
					if (owner.RayPick(src_p,one.D,height,collide::rqtBoth)){	
						...
						owner.RenewItem		(one,p->max_distance-_sqrt(dist_sqr),FALSE);		// fly ...
		...
		Fvector		pos_trail;	pos_trail.mad	(pos_head,one.D,-p->drop_length*factor_visual);
...

	{
		...
		u32						vCount_Lock		= p->particles_cache*DM_Drop->number_vertices;
		u32						iCount_Lock		= p->particles_cache*DM_Drop->number_indices;
		...
		while (P)	{
			...
			{
				// Build matrix
				float scale			=	P->time / p->particles_time;
