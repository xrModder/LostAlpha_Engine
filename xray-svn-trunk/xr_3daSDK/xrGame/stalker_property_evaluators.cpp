...
_value_type CStalkerPropertyEvaluatorShouldThrowGrenade::evaluate	()
{
	...
	return						(false);
#else // #if 1
//	Msg("Starting grenade evaluator for %s", object().cName().c_str());
	if (m_storage->property(eWorldPropertyStartedToThrowGrenade))
		return					(true);
//	Msg("1. grenade evaluator for %s", object().cName().c_str());
...
//	Msg("7. grenade evaluator for %s", object().cName().c_str());
	...
	if (!mem_object.m_object)
		return					(false);
//	Msg("8. grenade evaluator for %s", object().cName().c_str());
	...
	if (object().Position().distance_to_sqr(position) < _sqr(min_throw_distance))
		return					(false);
//	Msg("9. grenade evaluator for %s", object().cName().c_str());
	if (!object().agent_manager().member().can_throw_grenade(position))
		return					(false);
//	Msg("10. grenade evaluator for %s", object().cName().c_str());
	// setup throw target
	object().throw_target		(position, enemy_vertex_id , const_cast<CEntityAlive*>(enemy));
//	Msg("11. grenade evaluator for %s", object().cName().c_str());
	...
	if (object().inventory().ItemFromSlot(GRENADE_SLOT) == object().inventory().ActiveItem())
		return					(true);
//	Msg("12. grenade evaluator for %s", object().cName().c_str());
	// do not throw grenades when throw trajectory is obstructed
	if (!object().throw_enabled())
		return					(false);
//	Msg("13. grenade evaluator for %s", object().cName().c_str());
	// do throw grenade
	return						(true);
#endif // #if 1
}
