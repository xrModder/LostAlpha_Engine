...
bool CGhostBossFastGravi::check_start_conditions()
{
...
void CGhostBossFastGravi::activate()
{
	CGhostBoss *GhostBoss = smart_cast<CGhostBoss *>(m_object);
	m_man->subscribe	(this, ControlCom::eventTAChange);
	m_object->com_man().ta_activate(GhostBoss->anim_triple_gravi);

	m_object->dir().face_target(m_object->EnemyMan.get_enemy());
}

void CGhostBossFastGravi::deactivate()
{
	m_man->unsubscribe					(this, ControlCom::eventTAChange);
}

void CGhostBossFastGravi::on_event(ControlCom::EEventType type, ControlCom::IEventData *data)
{
...
void CGhostBossFastGravi::process_hit()
{
