...
//		CurrentGameUI()->UIMainIngameWnd->ZoneCounter().SetText("");
	...
	case GAME_PHASE_INPROGRESS:
		{
			...
			if(local_player && !local_player->IsSkip())
			{
				...
				{
					m_bFirstRun = FALSE;
					if (!Level().IsDemoPlay() && Level().CurrentEntity())
					{
						VERIFY( m_game_ui );
		...
		//			GetActiveVoting();
				...
				{
					string256 MoneyStr;
					xr_sprintf(MoneyStr, "%d", local_player->money_for_round);
	...
	if (m_game_ui->m_pInventoryMenu && m_game_ui->m_pInventoryMenu->IsShown() && !CanCallInventoryMenu())
		m_game_ui->m_pInventoryMenu->ShowDialog(true);
	...
	if (kINVENTORY == key )
	{
		if (Level().CurrentControlEntity() && smart_cast<CActor*>(Level().CurrentControlEntity()))
		{
			if (m_game_ui)
			{
				...
				{
					m_game_ui->m_pInventoryMenu->HideDialog();
				}
				else
				{
