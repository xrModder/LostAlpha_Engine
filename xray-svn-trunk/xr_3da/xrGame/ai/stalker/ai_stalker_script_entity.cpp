...
void CAI_Stalker::SetFireForced(CGameObject *obj)
{
	Fvector	enemy_position = obj->Position();
	Fvector	position = Position();
	Fvector	direction = Fvector().sub(enemy_position, position);
	...
	if (angle_difference(-yaw, current_angles.current.yaw) > PI_DIV_8)
	{
		
		distance = enemy_position.distance_to(position);
		select_queue_params(distance, min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
		CObjectHandler::set_goal(eObjectActionAimReady1, best_weapon(), min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
		return;
	}
	
	distance = enemy_position.distance_to(position);
	select_queue_params(distance, min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
	CObjectHandler::set_goal(eObjectActionFire1, best_weapon(), min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
	sight().setup(CSightAction(obj, true, true));
}


void CAI_Stalker::select_queue_params(const float &distance, u32 &min_queue_size, u32 &max_queue_size, u32 &min_queue_interval, u32 &max_queue_interval) 
{
	/**/
	int									weapon_type = 6;
	if (best_weapon())
	...
	switch (weapon_type) {
		...
		default : {
			if (distance > 30.f) {
				min_queue_size					= min_queue_size_far();
				max_queue_size					= max_queue_size_far();
				min_queue_interval				= min_queue_interval_far();
				max_queue_interval				= max_queue_interval_far();
			}
			else
				if (distance > 15.f) {
					min_queue_size				= min_queue_size_medium();
					max_queue_size				= max_queue_size_medium();
					min_queue_interval			= min_queue_interval_medium();
					max_queue_interval			= max_queue_interval_medium();
				}
				else {
					min_queue_size				= min_queue_size_close();
					max_queue_size				= max_queue_size_close();
					min_queue_interval			= min_queue_interval_close();
					max_queue_interval			= max_queue_interval_close();
				}
		}
	}
}